...
class TimeLoggingCallback(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ...
        :param pl_module: LightningModule instance.
        """
        pl_module.log_dict({'time_eval': time.time() - self.valid_start, 
                            'step': float(pl_module.current_epoch)}, sync_dist=True)
    ...
    def on_test_epoch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ...
        :param pl_module: LightningModule instance.
        """
        pl_module.log_dict({'time_test': time.time() - self.test_start, 
                            'step': float(pl_module.current_epoch)}, sync_dist=True)
    ...
    def on_train_epoch_end(
        ...
        """
        pl_module.log_dict({'time_epoch': time.time() - self.train_start, 
                            'step': float(pl_module.current_epoch)}, sync_dist=True)
