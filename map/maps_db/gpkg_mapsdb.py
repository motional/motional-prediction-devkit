...
from map.maps_db import layer_dataset_ops
from map.maps_db.imapsdb import IMapsDB
from map.maps_db.layer import MapLayer
from map.maps_db.metadata import MapLayerMeta
...
class GPKGMapsDB(IMapsDB):
    ...
    def __init__(self, map_version: str, map_root: str) -> None:
        ...
        # currently hard-coded for metadata file
        version_file = open(os.path.join(self._map_root, f"nuplan-maps-v1.0.json"))  # get blob and save to disk
    ...
    # ----- JS commented out for simplicity
    ...
    def load_vector_layer(self, location: str, layer_name: str) -> gpd.geodataframe:
        ...
        if not os.path.exists(path_on_disk):
            raise FileNotFoundError(f"Map does not exsist at {path_on_disk}")
            #---- JS commented out for simplicity
