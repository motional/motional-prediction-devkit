...
from map.abstract_map.abstract_map import AbstractMap, MapObject
from map.abstract_map.maps_datatypes import SemanticMapLayer, TrafficLightStatusData, TrafficLightStatusType
from map.nuplan_map.utils import (
...
class LaneSegmentConnections:
    ...
    def to_vector(self) -> List[List[int]]:
        """
        Returns data in vectorized form.
        :return: vectorized lane segment connections as [num_lane_segment, 2].
        """
...
def merge_vector_map(coords_a, conns_a, ids_a, 
                    coords_b, conns_b, ids_b) -> Tuple[List[List[List[float]]], List[Tuple[int, int]], List[str]]:
    """
    merge two vector maps a (coords_a, conns_a, ids_a) and b (coords_b, conns_b, ids_b)
    :param coords_a: List[List[List[float]] lane coordinates for map a 
    :param conns_a: List[Tuple[int, int]] lane connection (start, end) for map a 
    :param ids_a: List[str] lane ids for map a 
    :param coords_b: List[List[List[float]] lane coordinates for map b
    :param conns_b: List[Tuple[int, int]] lane connection (start, end) for map b
    :param ids_b: List[str] lane ids for map b
    :return : result of a union b, in the form of coords, conns and ids Tuple[List[List[List[float]]], List[Tuple[int, int]], List[str]]
    """
    num_lanes_a = len(ids_a)
    ids_a_set, ids_b_set = set(ids_a), set(ids_b)
    new_ids = set(ids_b).difference(ids_a_set)
    coords_merged, conns_merged, ids_merged = copy.deepcopy(coords_a), copy.deepcopy(conns_a), copy.deepcopy(ids_a)

    ids_merged += list(new_ids)
    # process new (b) ids
    # append each to the end of merged lane coordinates
    for i in range(num_lanes_a, len(ids_merged)):
        id_b = ids_merged[i]
        original_b_index = ids_b.index(id_b)
        coords_merged.append(coords_b[original_b_index][:])

    # merge connections
    for conn_b in conns_b:
        start, end = conn_b
        new_start, new_end = start, end

        # skip if both nodes are already in conn_a
        if ids_b[start] in ids_a_set and ids_b[end] in ids_a_set:
            continue

        # if the start/end node is in b, get the new index and append
        if ids_b[start] in new_ids:
            new_start = ids_merged.index(ids_b[start])
        if ids_b[end] in new_ids:
            new_end = ids_merged.index(ids_b[end])
        conns_merged.append((new_start, new_end))

    return coords_merged, conns_merged, ids_merged
